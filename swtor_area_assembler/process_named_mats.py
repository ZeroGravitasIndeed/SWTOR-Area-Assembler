...
class SWTOR_OT_process_named_mats(bpy.types.Operator):

    bl_label = "Process Named Materials"
    bl_idname = "swtor.process_named_mats"
    ...
    use_overwrite_bool: bpy.props.BoolProperty(
        ...
        description='Processes the selected objects\' materials even if they have a SWTOR shader already,\neffectively "regenerating" those ones. Useful when the add-on is updated with\nnew or better SWTOR shaders, for reprocessing existing objects',
        default = False,
        options={'HIDDEN'}
        )
...
def register():
    ...
    bpy.utils.register_class(SWTOR_OT_process_named_mats)
...
def unregister():
    ...
    bpy.utils.unregister_class(SWTOR_OT_process_named_mats)
