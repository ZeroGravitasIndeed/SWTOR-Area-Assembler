...
class SWTOR_OT_exclude_include_collections(bpy.types.Operator):
    bl_idname = "swtor.exclude_include_collections"
    bl_label = "Exclude/Include hierarchies of Collections From the View Layer"
    ...
    def execute(self, context):
        self.recursive = bpy.context.scene.SAAEAC_recursive
...
def register():
    bpy.utils.register_class(SWTOR_OT_exclude_include_collections)
    
    bpy.types.Scene.SAAEAC_recursive = bpy.props.BoolProperty(
        name="Affect Selected Collections' Hierarchies",
        description="Process Collections' hierarchies of children Collections, if any",
        default=True
    )
...
def unregister():
    bpy.utils.unregister_class(SWTOR_OT_exclude_include_collections)

    del bpy.types.Scene.SAAEAC_recursive
